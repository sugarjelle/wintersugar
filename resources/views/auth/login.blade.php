    ...
    <div class="w-full max-w-md bg-white border border-gray-200 rounded-xl shadow-sm p-8">
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center">
                <input id="remember_me" type="checkbox" name="remember"
                    class="h-4 w-4 text-gray-800 border-gray-300 rounded focus:ring-gray-400">
            ...
            <div>
                <button type="submit"
                    class="w-full bg-gray-800 hover:bg-black text-white font-semibold py-2 rounded-lg transition duration-200">
                    Login
                </button>
            </div>
        </form>
        ...
        <div class="text-center text-sm text-gray-600 mt-6 space-y-2">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-gray-800 hover:underline">Forgot your password?</a><br>
            @endif
            <span>Don't have an account?
                <a href="{{ route('register') }}" class="text-gray-800 font-medium hover:underline">Register</a>
            </span>
        </div>
    </div>
